...
(function($)
{
	...
	$.fn.advancedSearchField = function(options)
	{
		...
		function searchInNav(s)
		{
			...
			if (nbMatches > 0)
			{
				var output = '<p class="results-count"><strong>'+nbMatches+'</strong> ite'+((nbMatches > 1) ? 'ns' : 'm')+ ' encontra'+((nbMatches > 1) ? 'dos' : 'do')+'</p>';
	...
	$.fn.advancedSearchField.defaults = {
		 ...
		 * @var string
		 */
		tipFocus: 'Entre com sua busca.',
		
		/**
		 * Message is search is too short
		 * @var string
		 */
		tipTooShort: 'Digite pelo menos 2 caracteres',
		 ...
		 * @var string
		 */
		messageNoMatches: 'Nenhum resultado encontrado',
		
		/**
		 * Message when the search request is done
		 * @var string
		 */
		messageSearchDone: 'N&atilde;o encontrou? <a href="#">Tente a busca avan&ccedil;ada</a>',
		
		/**
		 * Full message if an error occurs while loading results
		 * @var string
		 */
		messageErrorFull: 'Erro ao carregar os resultados. Por favor tente novamente.',
		
		/**
		 * Status message if an error occurs while loading results
		 * @var string
		 */
		messageError: 'Erro na busca.',
		
		/**
		 * Title of the result section showing template nav items
		 * @var string
		 */
		titleTemplateResult: 'Funcionalidades'
	};
